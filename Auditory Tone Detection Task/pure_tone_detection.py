...
INSTRUCT_TXT7 = (
    ...
    "the noise by responding with the number keys."
)
...
expName = 'pure_tone_detection'  # from the Builder filename that created this script
expInfo = {'Participant': '', 'Session': '1', 'Condition': ['Quiet', 'Simultaneous', 'Backward'], 'Calibration': '0.075857758', 'Practice': ['Yes', 'No'], 'Instructions': ['Yes', 'No']}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/workingman/Documents/ASD_and_Synesthesia/Python/psychopy_shared_github/Open-source-Auditory-Tone-Detection-Task/Auditory Tone Detection Task/pure_tone_detection.py',
...
#Determine whether instructions should be run
if expInfo['Instructions'] == 'Yes':
    instructionsToggle = 1
if expInfo['Instructions'] == 'No':
    instructionsToggle = 0
# OVERRIDE (specific for experiment runs in Stockholm,
# for Janina Neufeld's synesthesia project):
# _always_ skip instructions if non-'Quiet' version of
# experiment (condition) is run
if expInfo['Condition'] != 'Quiet':
    instructionsToggle = 0
...
instructionsLightBulb1 = visual.ImageStim(
    ...
    image='lightBulb_OFF.png', mask=None, anchor='center',
...
instructionsLightBulb1_4 = visual.ImageStim(
    ...
    image='lightBulb_ON.png', mask=None, anchor='center',
    ori=0, pos=(-0.5, 0), size=(0.4, 0.4),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=0.0)
...
instructions_loop = data.TrialHandler(nReps=instructionsToggle, method='sequential', 
...
for thisInstructions_loop in instructions_loop:
    ...
    psychopy_input_value = ((calibration)/(10**(60/20))*(10**(desired_db/20)))
...
# completed instructionsToggle repeats of 'instructions_loop'
...
for thisPracticeTrial in practiceTrials:
    ...
    routineTimer.add(3.000000)
    # update component parameters for each repeat
    #set desired dB of the practice trials to 60 db SPL 
    desired_db = int(60)
    psychopy_input_value = ((calibration)/(10**(60/20))*(10**(desired_db/20)))
    ...
    if expInfo['Condition'] == 'Quiet': #if the Condition is Quiet, turn off background noise 
        ...
        noise_db = int(75) 
        calibration = float(calibration)
        psychopy_input_value_for_noise = ((calibration)/(10**(60/20))*(10**(noise_db/20)))
    # keep track of which components have finished
    ...
    # the Routine "practice_decisionSpace" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    if decisionKey_2.keys[0] == str(corrAns):
       feedback = CORRECT_TXT
    
    if decisionKey_2.keys[0] != str(corrAns):
       feedback = INCORRECT_TXT
...
for level, condition in trials:
    ...
    trials.addResponse(decisionKey.corr, level)
    ...
    # the Routine "decisionSpace" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
